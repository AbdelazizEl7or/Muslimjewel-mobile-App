...
function PrayTimes(method) {
  ...
  return {
    ...
    getTimes: async function (date, coords, timezone, dst, format) {
      coords = coords || [23.31667, 58.01667];
      lat = 1 * coords[0];
      lng = 1 * coords[1];
      ...
      return await this.computeTimes();
    },
    ...
    computeTimes: async function () {
      ...
      return await this.modifyFormats(times);
    },
    ...
    modifyFormats: async function (times) {
      times.dhuhr += 0.08;
      times.asr += 0.08;
      times.maghrib += 0.08;
      times.isha += 0.09;
      ...
      if (azt.azNumber1) times.fajr += (+azt.azNumber1 - 60) / 60;
      if (azt.azNumber2) times.dhuhr += (+azt.azNumber2 - 60) / 60;
      if (azt.azNumber3) times.asr += (+azt.azNumber3 - 60) / 60;
      if (azt.azNumber4) times.maghrib += (+azt.azNumber4 - 60) / 60;
      if (azt.azNumber5) times.isha += (+azt.azNumber5 - 60) / 60;
      for (var i in times)
        times[i] = this.getFormattedTime(times[i], timeFormat, null);
      return times;
    },
